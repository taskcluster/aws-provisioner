...
/** Subclass of Entity for representation of a workerType */
var WorkerType = function(entity) {
...
Entity.subClass(WorkerType, nconf.get('provisioning:azureWorkerTypeTable'), [
  {
    ...
    // This is always hardcoded to 'definition', so we can use the same
    // partition key for other related data if we should ever need this.
    key:              'RowKey',
    type:             'string',
    hidden:           true
  }, {
...
WorkerType.create = function(properties) {
  properties.RowKey = 'definition';
  return Entity.create(properties, WorkerType);
};

/** Load WorkerType */
WorkerType.load = function(workerType) {
  return Entity.load(workerType, 'definition', WorkerType);
};

/** Load all registered worker-types */
