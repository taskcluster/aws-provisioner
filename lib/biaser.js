 ...
 * of one.
 */
Biaser.prototype.getBias = function(region, zone, type) {
  assert(region);
  assert(zone);
  assert(type);
  ...
  if (this.__biasTable[region] &&
      this.__biasTable[region][zone] &&
      this.__biasTable[region][zone][type]) {
    let info = this.__biasTable[region][zone][type];
    ...
    if (info.spawns === 0 && info.kills === 0) {
      return this.__emptyComboBias;
    }

    let killrate = info.kills / info.spawns;
 ...
 */
Biaser.prototype.fetchBiasInfo = async function(azinfo, types) {
  ...
  let results = await this.influx.query(spotKillQuery + ';' + spotFulfilledQuery);
  let kills = results[0];
  ...
  for (let killpt of kills.points) {
    ...
    if (!biasTable[region][zone]) {
      biasTable[region][zone] = {};
    }
    if (!biasTable[region][zone][type]) {
      biasTable[region][zone][type] = {
        kills: 0,
        spawns: 0
      };
    }
    biasTable[region][zone][type].kills = killcount;
  }
  ...
  for (let spawnpt of spawns.points) {
    ...
    biasTable[region][zone][type].spawns = spawncount;
  }
