...
/**
 * Return a list of every scope for every region and instance type combination.
 * Currently, scope [1,2] is considered unique compared to [2,1].  it would be
 * nice if we could convert the array of arrays into a set of sets for
 * comparisons
 */
WorkerType.scopes = function (worker) {
  // Generate the complete list of scopes;
  let scopes = [];

  // Take an object with a .scopes property and list add all of its (mostly)
  // unique scopes to the list of scopes to return
  var y = function(x) {
    for (let scope of x.scopes) {
      if (typeof scope === 'string') {
        scope = [scope];
      }
      let hasThisScope = false;
      for (let knownScope of scopes) {
        if (lodash.isEqual(scope, knownScope)) {
          hasThisScope = true;
          break;
        }
      }
  ...
  for (let region of worker.regions) {
    y(region);
  }

  // Instance Type specific scopes
  for (let instanceType of worker.instanceTypes) {
    y(instanceType);
  }
...
/**
 * Make the scopes method available on instances as well
 */
WorkerType.prototype.scopes = function () {
  return Worker.scopes(this);
};
...
WorkerType.prototype.determineSpotBids = function (managedRegions, pricing, change) {
  ...
  while (change > 0) {
    ...
    if (cheapestPrice <= this.maxPrice) {
      ...
      throw new Error(this.workerType + ' could not create a bid less than the max price');
    }
