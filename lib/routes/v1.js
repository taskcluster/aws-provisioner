...
let SLUGID_PATTERN = /^[a-zA-Z0-9-_]{22}$/;
...
api.declare({
  ...
  // Figure out which scopes the client needs
  let requiredScopes = this.WorkerType.scopes(input);
  // Now check for them if there are any
  if (requiredScopes.length > 0) {
    if (!req.satisfies(requiredScopes)) {
      return;
    }
  }
