...
class AwsManager {
  ...
  async update () {
    ...
    await Promise.all(_.map(this.ec2, async (ec2, region) => {
      debug('running aws state promises');
      ...
      debug('killing stalled instances and spot requests');
      await this.killCancel(region, [], stalledSRIds);
      debug('killed stalled instances and spot requests');
  ...
  async createKeyPair (workerName) {
    ...
    debug('ensuring %s is present', keyName);
    await Promise.all(_.map(this.ec2, async (ec2, region) => {
    ...
    debug('ensured %s is present', keyName);
