...
let load = loader({
  ...
  Secret: {
    ...
    setup: async ({cfg, monitor}) => {
      let Secret = secret.setup({
        accountId: cfg.azure.accountId,
        tableName: cfg.app.secretTableName,
        signingKey: cfg.app.tableSigningKey,
        cryptoKey: cfg.app.tableCryptoKey,
        credentials: sasCredentials({
          accountId: cfg.azure.accountId,
  ...
  ec2manager: {
    ...
    setup: async ({cfg}) => {
      ...
      let clientClass = taskcluster.EC2Manager;

      // We only want to build a custom client when we're running against
      // a localhost-ed copy of ec2-manager
      if (urllib.parse(ec2ManagerBaseUrl).hostname === 'localhost') {
        let reference = await request.get(ec2ManagerBaseUrl + '/internal/api-reference');
        reference = JSON.parse(reference);
        clientClass = taskcluster.createClient(reference);
      }
      ...
      return new clientClass({
        ...
        credentials: cfg.taskcluster.credentials,
        // wait 15 seconds, hoping to split the difference between giving up
        // too early (2s) and spending too long on provisioner iterations
        timeout: 15 * 1000,
      });
    },
  },
